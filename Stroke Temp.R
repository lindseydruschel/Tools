...
file_path <- "/Users/druschel/Documents/R/Temp App/LND XL 12-12-24 Log.csv"
...
# Convert Time to POSIXct using UTC (raw data assumed to be on 1970-01-01)
data$Time <- as.POSIXct(data$Time, format = "%H:%M:%S", tz = "UTC")
midnight <- as.POSIXct("1970-01-01 00:00:00", tz = "UTC") #change this is the picolog no longer autosets as 01-01-1970
    ...
    if (input$channel_tab == "Channel B") {
      ...
      if (nrow(selected_data) > 1) {
        ...
        new_row <- data.frame(
          ...
          Time_to_Reached = ifelse(is.na(stats_calc$time_to_reach),
                                   "Never Reached",
                                   paste0(round(stats_calc$time_to_reach, 1), " sec")),
